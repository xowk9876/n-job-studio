...
/**
 * 스트레스 DSR 가산금리 (%p) — 2026년 하반기(2026.7.1~12.31) 적용분
 *
 * 근거: 은행연합회 「2026년 하반기 스트레스 DSR 운영방안」(2026.6.30 발표,
 *      금융위원회 스트레스 DSR 행정지도 변경 시행). 스트레스 DSR 3단계는
 *      2025.7.1부터 시행 중이며, 지방 비규제지역 주담대만 2단계가 유예 연장됨.
 *
 * 산정식: 최종 가산금리 = 스트레스 금리 × 기본 적용비율 × 대출유형별 적용비율
 *  - 스트레스 금리 = 예금은행 가계대출 신규취급 가중평균금리의 과거 5년 최고치
 *    (2022.12, 5.64%) − 현재 금리. 하한 1.5% ~ 상한 3.0% 범위에서 매년 6·12월 고시.
 *    2026년 하반기 산정값은 하한인 1.5%.
 *  - 대출유형별 적용비율은 변동형 100% 기준. 혼합형·주기형은 고정금리 기간이
 *    길수록 낮아지고, 고정기간(또는 금리변동주기)이 만기의 70% 이상이면 미적용(0%).
 *
 * ※ 매년 6월·12월 재고시되므로 갱신 시점마다 재검증 필요.
 */
export const STRESS_DSR_RATES = {
  /** 수도권·규제지역 주담대: 스트레스 금리 3.0%(10·15 부동산 대책) × 3단계 100% */
  metroMortgage: 3.0,
  /** 지방 비규제지역 주담대: 1.5% × 2단계 50% — 3단계 유예 2026.12.31까지 */
  localMortgage: 0.75,
  /** 신용대출·기타대출: 1.5% × 3단계 100% (신용대출은 총 대출잔액 1억원 초과 시만 대상) */
  creditLoan: 1.5,
} as const
