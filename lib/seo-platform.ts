...
export const guideRssBodies: Record<string, string> = {
  '2026-salary-tax-guide': `2026년 연봉 실수령액은 국민연금(근로자 4.75%), 건강보험(3.595%), 장기요양(건보료의 13.14%), 고용보험(0.9%)과 근로소득세·지방소득세를 공제한 뒤 산출합니다. 국민연금은 기준소득월액 상한 659만 원(2026.7~2027.6) 기준 월 공제 상한 313,025원입니다. 비과세 식대(연 240만 원), 부양가족 수, 2026년 자녀세액공제(첫째 25만·둘째 30만·셋째 이후 40만)에 따라 같은 연봉이라도 실수령액 차이가 큽니다. 연봉 5,000만 원·독신·식대 240만 원 반영 시 월 실수령액은 약 340만 원대입니다. 머니핏 연봉 계산기에서 바로 시뮬레이션할 수 있습니다.`,
  ...
  'isa-vs-regular-savings': `ISA(개인종합자산관리계좌)는 일반형 비과세 한도 200만 원, 서민형·농어민형 400만 원까지 이자·배당이 비과세이고 초과분은 9.9% 분리과세됩니다. 일반 계좌는 15.4% 원천징수가 적용되어 10년 복리 시 ISA가 백만 원 이상 유리할 수 있습니다. 납입한도는 연 2,000만 원·총 1억 원, 의무가입 3년입니다. 머니핏 적금 계산기에서 ISA 비교 시뮬레이션을 함께 확인하세요.`,
  ...
  'lotto-tax-guide': `로또 당첨금은 소득세법상 기타소득으로, 200만 원 이하는 비과세, 200만 원 초과~3억 원 이하는 22%(소득세 20%+지방세 2%), 3억 원 초과분은 33% 원천징수됩니다. 1등 당첨금은 NH농협은행 본점에서, 2등과 200만 원 초과 3등은 농협은행 전국 지점에서 수령하며, 200만 원 이하는 로또 판매점에서 수령 가능합니다. 당첨금은 예금자보호 1인당 1억 원(2025.9.1~) 한도를 고려해 분산 보관하는 것이 안전합니다.`,
}
...
export const calculatorRssBodies: Record<string, string> = {
  ...
  '/savings':
    '예금·적금 단리·복리 이자와 15.4% 이자소득세 차감 후 세후 만기 수령액을 계산합니다. ISA 일반형 200만 원·서민형 400만 원 비과세 한도 비교가 가능합니다.',
  '/jeonse':
